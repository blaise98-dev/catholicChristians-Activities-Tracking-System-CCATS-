		...
		<div class="row-fluid">
			<div class="span9" id="content">
				<div class="row-fluid">
					...
					<?php
					$count_events = mysqli_query($conn, "SELECT * FROM `event` WHERE Date >= CURDATE();");
								...
								<form action="" method="post">
									<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-hover table-responsive table-bordered" id="example">
										...
										<tbody>
											<!-----------------------------------Content------------------------------------>
											<?php
											$members_query = mysqli_query($conn, "SELECT * FROM `event` WHERE Date >= CURDATE();") or die(mysqli_error($conn));
											$i = 0;
