            ...
            <div class="controls">
              <p>
                <select class="input focused" name="ministry" id="focusedInput" required="required" type="text">
                  ...
                  $retrieveevents = mysqli_query($conn, $retrieve) or die(mysqli_error($conn));
                  while ($events = mysqli_fetch_assoc($retrieveevents)) {
                  ?>
                    <option value="">Select Event Attended</option>
                    <option value="<?php echo $events['Title']; ?>"><?php echo $events['Title']; ?></option>
<?php } ?>
                </select>
              </p>
            </div>
...
if (isset($_POST['save'])) {
  ...
  $mobile = $_POST['mobile'];
  $query = @mysqli_query($conn, "select * from visitor where  mobile = '$mobile' and visitor.host_parish='" . $_SESSION['id'] . "'  ") or die(mysqli_error($conn));
  ...
  <?php
    ...
    mysqli_query($conn, "insert into Visitor(id,fname,lname,Gender,birthday,residence,host_parish,pob,ministry,mobile,thumbnail) 
values('','$fname','$lname','$Gender','$birthday','$residence','" . $_SESSION['id'] . "','$pob','$ministry','$mobile','uploads/none.png')") or die(mysqli_error($conn));
