...
<body>
    ...
    <div class="container-fluid">
                ...
                <div class="row-fluid">
                    ...
                    <div id="block_bg" class="block">
                        <div class="navbar navbar-inner block-header">
                            <div class="muted pull-left"><i class="icon-reorder icon-large"></i> Christians requesting to leave our parish </div>
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                                        ...
                                        <tbody>
                                            ...
                                            while ($row = mysqli_fetch_array($student_query)) {
                                                $username = $row['id'];
                                                $destination=$row['newparish'];
                                                $previous_query = mysqli_query($conn, "SELECT `admin`.username FROm `admin`WHERE `admin`.admin_id='$destination'") or die(mysqli_error($conn));
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a tabindex="-1" href="#modaloutgoing" class="text-primary" data-toggle="modal">Approve</a>
                                                    </td>
                                                </tr>
        ...
        <?php
 if (isset($_POST['Authorize_transfer'])) {
    error_reporting(0);                                 
    mysqli_query($conn, "update tbltransfer set `status` = 'outgoing',sent_status=1 where tbltransfer.id = $username and previousParish=$session_id ") or die(mysqli_error($conn));

    echo '<meta http-equiv="refresh" content="0;url=outgoingChristians.php">';
    header('Location:outgoingChristians.php');

}?>

        <!-- modal -->
        <div id="modaloutgoing" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="mymodalLabel" aria-hidden="true">
            ...
            <div class="modal-footer">
                <button class="btn" data-dismiss="modal" aria-hidden="true"><i class="icon-remove icon-large"></i>
                    Close</button>
                <button class="btn btn-info" name="Authorize_transfer"><i class="icon-save icon-large"></i> Approve</button>
            </div>
            </form>
        </div>
        <!-- end  modal -->
